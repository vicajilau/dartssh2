...
- Documented what `handshakeTimeout` and `authTimeout` actually bound. Both are null by default, so out of the box nothing stops a server that accepts the connection and then goes quiet from leaving the client waiting for as long as the socket stays open, and `keepAliveInterval` does not cover it because it only starts once the connection is up. Both are also deadlines over the whole phase rather than inactivity timers, and the callbacks run inside them: `onVerifyHostKey` counts against the first, `onPasswordRequest` and `onUserInfoRequest` against the second. A value picked to bound an unresponsive server will cut off a user who is slow reading a fingerprint or typing a password, which the README's own 15 second example would have done [#247].
...
[#247]: https://github.com/vicajilau/dartssh2/pull/247
